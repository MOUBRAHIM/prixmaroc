  ...
  <View style={styles.row}>
    ...
    <View style={styles.votes}>
      <TouchableOpacity disabled={voting} onPress={() => onVote(p.id, 1)} style={styles.voteBtn} hitSlop={8}>
        <Ionicons name={p.my_vote === 1 ? 'thumbs-up' : 'thumbs-up-outline'} size={18} color={p.my_vote === 1 ? C.primary : '#93A09A'} />
        <Text style={[styles.voteNum, p.my_vote === 1 && { color: C.primary }]}>{p.upvotes}</Text>
      </TouchableOpacity>
      <TouchableOpacity disabled={voting} onPress={() => onVote(p.id, -1)} style={styles.voteBtn} hitSlop={8}>
        <Ionicons name={p.my_vote === -1 ? 'thumbs-down' : 'thumbs-down-outline'} size={18} color={p.my_vote === -1 ? '#D0402F' : '#93A09A'} />
        <Text style={[styles.voteNum, p.my_vote === -1 && { color: '#D0402F' }]}>{p.downvotes}</Text>
      </TouchableOpacity>
    </View>
  </View>
);
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.modalWrap}>
        <View style={styles.sheet}>
          <View style={styles.sheetHeader}>
            ...
            <TouchableOpacity onPress={onClose} hitSlop={8}><Ionicons name="close" size={24} color="#5A6A61" /></TouchableOpacity>
          </View>
          <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{ paddingBottom: 20 }}>
            ...
            <TextInput style={styles.input} value={itemName} onChangeText={setItemName}
              placeholder="Ex : Tomates, Sardines…" placeholderTextColor="#93A09A" />
            ...
            <View style={styles.priceRow}>
              <TextInput style={[styles.input, { flex: 1, marginBottom: 0 }]} value={price} onChangeText={setPrice}
                placeholder="0" placeholderTextColor="#93A09A" keyboardType="decimal-pad" />
            ...
            <TextInput style={styles.input} value={neighborhood} onChangeText={setNeighborhood}
              placeholder="Ex : Souk Derb Ghallef" placeholderTextColor="#93A09A" />
            ...
            <TextInput style={[styles.input, { height: 64 }]} value={note} onChangeText={setNote}
              placeholder="Qualité, marchand…" placeholderTextColor="#93A09A" multiline />
...
const styles = StyleSheet.create({
  ...
  content: { padding: 16 },
  intro: { backgroundColor: '#E4F1EA', borderRadius: 14, padding: 14, marginBottom: 14, borderWidth: 1, borderColor: '#95CEB4' },
  introTitle: { fontSize: 17, fontWeight: '800', color: '#14211B', marginBottom: 4 },
  introText: { fontSize: 13, color: '#073A2B', lineHeight: 18 },
  cityBox: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: '#fff', borderRadius: 12, borderWidth: 1, borderColor: '#EBE3D7', paddingHorizontal: 12, marginBottom: 12 },
  cityInput: { flex: 1, paddingVertical: 12, fontSize: 15, color: '#14211B' },
  catRow: { gap: 8, paddingBottom: 4, paddingRight: 8 },
  catChip: { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: '#fff', borderRadius: 20, paddingHorizontal: 12, paddingVertical: 8, borderWidth: 1, borderColor: '#EBE3D7' },
  catChipActive: { backgroundColor: C.primary, borderColor: C.primary },
  catChipIcon: { fontSize: 14 },
  catChipLabel: { fontSize: 13, fontWeight: '700', color: '#47564F' },
  sectionTitle: { fontSize: 15, fontWeight: '800', color: '#14211B', marginTop: 18, marginBottom: 10 },
  medCard: { width: 130, backgroundColor: '#fff', borderRadius: 14, padding: 12, borderWidth: 1, borderColor: '#EBE3D7' },
  medIcon: { fontSize: 20 },
  medName: { fontSize: 13, fontWeight: '700', color: '#14211B', marginTop: 4 },
  medPrice: { fontSize: 18, fontWeight: '900', color: C.primary, marginTop: 4 },
  medUnit: { fontSize: 11, fontWeight: '600', color: '#93A09A' },
  medRange: { fontSize: 10, color: '#93A09A', marginTop: 2 },
  ...
  rowIcon: { fontSize: 22 },
  rowName: { fontSize: 14, fontWeight: '700', color: '#14211B' },
  rowMeta: { fontSize: 11, color: '#93A09A', marginTop: 2 },
  ...
  rowPrice: { fontSize: 16, fontWeight: '900', color: '#14211B' },
  rowUnit: { fontSize: 10, color: '#93A09A' },
  votes: { alignItems: 'center', gap: 2, marginLeft: 4 },
  voteBtn: { flexDirection: 'row', alignItems: 'center', gap: 3, paddingVertical: 2 },
  voteNum: { fontSize: 12, fontWeight: '700', color: '#93A09A' },
  empty: { alignItems: 'center', paddingVertical: 40, gap: 8 },
  emptyIcon: { fontSize: 40 },
  emptyText: { fontSize: 13, color: '#93A09A', textAlign: 'center', lineHeight: 19 },
  ...
  sheetTitle: { fontSize: 18, fontWeight: '800', color: '#14211B' },
  label: { fontSize: 13, fontWeight: '700', color: '#2E4139', marginTop: 12, marginBottom: 6 },
  optional: { fontWeight: '400', color: '#93A09A' },
  input: { backgroundColor: '#FBF7F1', borderRadius: 10, borderWidth: 1, borderColor: '#EBE3D7', paddingHorizontal: 12, paddingVertical: 11, fontSize: 15, color: '#14211B', marginBottom: 2 },
  segRow: { flexDirection: 'row', gap: 6 },
  seg: { flex: 1, alignItems: 'center', backgroundColor: '#FBF7F1', borderRadius: 10, paddingVertical: 10, borderWidth: 2, borderColor: '#EBE3D7', gap: 2 },
  segActive: { borderColor: C.primary, backgroundColor: '#E4F1EA' },
  segIcon: { fontSize: 18 },
  segLabel: { fontSize: 11, fontWeight: '700', color: '#5A6A61' },
  sugRow: { gap: 6, paddingVertical: 8 },
  sugChip: { backgroundColor: '#E4F1EA', borderRadius: 16, paddingHorizontal: 12, paddingVertical: 6, borderWidth: 1, borderColor: '#95CEB4' },
  sugText: { fontSize: 12, color: C.primary, fontWeight: '600' },
  priceRow: { flexDirection: 'row', alignItems: 'center', gap: 8, flexWrap: 'wrap' },
  madLabel: { fontSize: 13, fontWeight: '700', color: '#5A6A61' },
  unitRow: { flexDirection: 'row', gap: 4 },
  unitChip: { backgroundColor: '#F3EDE3', borderRadius: 8, paddingHorizontal: 10, paddingVertical: 8 },
  unitChipActive: { backgroundColor: C.primary },
  unitChipText: { fontSize: 12, fontWeight: '700', color: '#47564F' },
